...
export default function CampaignsPage() {
  ...
  const load = async () => {
    ...
    else setMessage('Não foi possível carregar as campanhas.');
  };
  useEffect(() => {
    void load();
  }, []);
  async function action(campaign: Campaign, operation: 'schedule' | 'pause' | 'resume' | 'cancel') {
    if (
      operation === 'cancel' &&
      !window.confirm(
        `Cancelar a campanha “${campaign.name}”? Os disparos futuros serão bloqueados.`,
      )
    )
      return;
    setMessage('');
    const response = await authFetch(`${API}/campaigns/${campaign.id}/${operation}`, {
      method: 'POST',
    });
  ...
  return (
    ...
    >
      ...
      {message ? (
        <p className={message === 'Campanha atualizada.' ? 'success' : 'error'}>{message}</p>
      ) : null}
      <section className="panel">
        <header className="panel-header">
          <div>
            <h2>Todas as campanhas</h2>
            <span className="muted">{items.length} campanhas cadastradas</span>
          </div>
        ...
        {items.length ? (
                  ...
                  <tr key={item.id}>
                    ...
                    <td>
                      <div className="cell-main">
                        ...
                        <small>
                          3ª: +{item.thirdIntervalDays ?? '—'} dias às {item.thirdStartTime ?? '—'}
                        </small>
                      </div>
                    </td>
                    <td>
                      ...
                      <div className="actions">
                        {item.status === 'DRAFT' ? (
                          <button
                            ...
                            onClick={() => void action(item, 'schedule')}
                          >
                            Programar
                          </button>
                        ) : null}
          ...
          <EmptyState
            ...
            description="Aprove uma importação para configurar a primeira campanha."
          />
        )}
      </section>
    </AppShell>
