...
type Campaign = { id: string; name: string; status: string; firstActionAt: string | null; secondIntervalDays: number | null; thirdIntervalDays: number | null; _count: { convocations: number } };
...
export default function CampaignsPage() {
  ...
  return <main className="shell"><Navigation current="/campanhas" /><section className="container"><p className="eyebrow">Operação</p><h1>Campanhas</h1><p className="muted">Programe, pause, retome ou cancele campanhas sem perder o histórico.</p>{message ? <p className={message === 'Campanha atualizada.' ? 'success' : 'error'}>{message}</p> : null}<section className="card table-wrap"><table><thead><tr><th>Campanha</th><th>Pacientes</th><th>1ª convocação</th><th>Status</th><th>Ações</th></tr></thead><tbody>{items.map(item => <tr key={item.id}><td><strong>{item.name}</strong><br /><span className="muted">2ª em {item.secondIntervalDays ?? '—'} dias · 3ª em {item.thirdIntervalDays ?? '—'} dias</span></td><td>{item._count.convocations}</td><td>{item.firstActionAt ? new Date(item.firstActionAt).toLocaleString('pt-BR') : '—'}</td><td><span className="badge">{item.status}</span></td><td className="actions">{item.status === 'DRAFT' ? <button className="button small" onClick={() => void action(item, 'schedule')}>Programar</button> : null}{['SCHEDULED', 'RUNNING'].includes(item.status) ? <button className="button secondary small" onClick={() => void action(item, 'pause')}>Pausar</button> : null}{item.status === 'PAUSED' ? <button className="button small" onClick={() => void action(item, 'resume')}>Retomar</button> : null}{!['CANCELLED', 'COMPLETED'].includes(item.status) ? <button className="button danger small" onClick={() => void action(item, 'cancel')}>Cancelar</button> : null}</td></tr>)}{items.length === 0 ? <tr><td colSpan={5} className="muted">Nenhuma campanha criada. Aprove uma importação para criar a primeira.</td></tr> : null}</tbody></table></section></section></main>;
}
